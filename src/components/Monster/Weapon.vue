...
<template>
  <div
    class="col-span-4 hidden cursor-pointer self-center truncate border-t-2 border-stone-800 px-1 pt-1 text-left lg:block xl:hidden"
    @click="displayProbs = !displayProbs">
    {{ weapon.name }} <span class="tracking-tighter grayscale">{{ weapon.icon }}</span>:
  </div>
  <div class="hidden cursor-pointer self-center border-stone-800 px-1 pb-1 text-left lg:block xl:hidden"
    @click="displayProbs = !displayProbs">
    Roll {{ numOfDice }}
  </div>

  <div dir="rtl"
    class="cursor-pointer self-center truncate border-t-2 border-stone-800 p-1 text-right lg:hidden xl:block"
    @click="displayProbs = !displayProbs">
    {{ weapon.name }} <span class="tracking-tighter grayscale">{{ weapon.icon }}</span>: Roll {{ numOfDice }}
  </div>

  <div class="cursor-pointer self-center border-t-2 border-stone-800 p-1 lg:border-0 lg:pt-0 xl:border-t-2 xl:pt-0"
    @click="displayProbs = !displayProbs">
    Hit: {{ lantern(hitOn) }}
  </div>
  <div
    class="hidden cursor-pointer self-center border-t-2 border-stone-800 p-1 lg:border-0 lg:pt-0 xl:border-t-2 xl:pt-0"
    @click="displayProbs = !displayProbs">
